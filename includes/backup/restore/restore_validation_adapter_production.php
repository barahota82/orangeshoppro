...
function orange_restore_validation_adapter_production_postcheck(
    ...
    $gates[] = orange_restore_validation_adapter_production_gate(
        'critical_row_counts',
        ORANGE_RESTORE_PRODUCTION_GATE_HARD,
        !$criticalMismatch && $criticalRowChecks !== [],
        !$criticalMismatch
            ? 'Critical row counts match validated staging.'
            : 'Critical row count mismatch vs staging.',
    ...
    $crossCountryErrors = orange_restore_validation_adapter_production_cross_country_checks($productionPdo);
    $gates[] = orange_restore_validation_adapter_production_gate(
    ...
    $gates[] = orange_restore_validation_adapter_production_gate(
        ...
        ORANGE_RESTORE_PRODUCTION_GATE_INFO,
        true,
        'Maintenance verified by caller before gate execution.',
        ['note' => 'Caller must verify maintenance before invoking postcheck.']
    );
 ...
 * @return array{ok:bool,checked:int,missing:list<string>,warnings:list<string>}
 */
function orange_restore_validation_adapter_production_required_uploads_check(
    ...
    $verifyPath = static function (string $dbPath) use ($projectRoot, &$missing, &$warnings, &$checked): void {
        ...
        if (str_starts_with($dbPath, '/uploads/')) {
            ...
            $dbPath = 'uploads/products/' . ltrim($dbPath, '/');
        }
        $abs = $projectRoot . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $dbPath);
        $checked++;
        if (!is_file($abs)) {
            $missing[] = $dbPath;
        }
    ...
    if (in_array('products', $productionTables, true)) {
        try {
            $st = $pdo->query(
                "SELECT main_image FROM products WHERE main_image IS NOT NULL AND TRIM(main_image) <> ''"
            );
            if ($st !== false) {
                while ($row = $st->fetch(PDO::FETCH_ASSOC)) {
                    if (is_array($row)) {
                        $verifyPath((string) ($row['main_image'] ?? ''));
                    }
                }
            }
        } catch (Throwable $e) {
            $warnings[] = 'products.main_image scan skipped: ' . $e->getMessage();
        }
    }

    if (in_array('product_images', $productionTables, true)) {
        try {
            $st = $pdo->query(
                "SELECT image_path FROM product_images WHERE image_path IS NOT NULL AND TRIM(image_path) <> ''"
            );
            if ($st !== false) {
                while ($row = $st->fetch(PDO::FETCH_ASSOC)) {
                    if (is_array($row)) {
                        $verifyPath((string) ($row['image_path'] ?? ''));
                    }
                }
            }
        } catch (Throwable $e) {
            $warnings[] = 'product_images scan skipped: ' . $e->getMessage();
        }
    }
