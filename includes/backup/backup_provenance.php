...
/**
 * Backup Provenance Metadata Registry — Stage 1 Backend sidecar.
 *
 * Layout under configured BackupRoot:
 ...
 * Never controls Health, Verify, DRV, Recoverable, or Restore eligibility.
 *
...
function orange_backup_provenance_env_execution_id(): ?string
{
    ...
    return preg_match(ORANGE_BACKUP_PROVENANCE_EXECUTION_ID_RE, $id) ? $id : null;
}
...
function orange_backup_provenance_begin_execution(string $backupRoot, array $fields): array
{
    ...
    $record = [
        ...
        'error_summary' => isset($fields['error_summary']) ? (string) $fields['error_summary'] : null,
    ];
 ...
 * @return array{ok:bool,error?:string}
 */
function orange_backup_provenance_write_package_record(
    string $backupRoot,
    string $relativeUnderV1,
    array $record
): array {
    $pathInfo = orange_backup_provenance_resolve_contained_path($backupRoot, $relativeUnderV1);
    if (empty($pathInfo['ok'])) {
        return ['ok' => false, 'error' => (string) ($pathInfo['error'] ?? 'path')];
    }
    $path = (string) $pathInfo['path'];
    $existing = orange_backup_provenance_read_json_file($path);
    if (is_array($existing)) {
        $exFp = trim((string) ($existing['package_fingerprint'] ?? ''));
        $newFp = trim((string) ($record['package_fingerprint'] ?? ''));
        $exId = (string) ($existing['package_id'] ?? '');
        $newId = (string) ($record['package_id'] ?? '');
        $exRel = (string) ($existing['relative_package_path'] ?? '');
        $newRel = (string) ($record['relative_package_path'] ?? '');
        if ($exId !== '' && $newId !== '' && $exId !== $newId) {
            return ['ok' => false, 'error' => 'duplicate_conflicting_package_id'];
        }
        if ($exRel !== '' && $newRel !== '' && $exRel !== $newRel) {
            return ['ok' => false, 'error' => 'duplicate_conflicting_relative_path'];
        }
        if ($exFp !== '' && $newFp !== '' && !hash_equals($exFp, $newFp)) {
            return ['ok' => false, 'error' => 'duplicate_conflicting_fingerprint'];
        }
    }

    return orange_backup_provenance_atomic_write_json($path, $record);
}
...
function orange_backup_provenance_record_full_package(string $backupRoot, array $fields): array
{
    ...
    return orange_backup_provenance_write_package_record(
        $backupRoot,
...
function orange_backup_provenance_record_country_package(string $backupRoot, array $fields): array
{
    ...
    $write = orange_backup_provenance_write_package_record(
        $backupRoot,
...
/**
 * Bind provenance for internal consumers (Stage 1: not attached to Backup Center list JSON).
 *
 * @param array<string, mixed> $packageSummary
 * @return array<string, mixed>
 */
function orange_backup_provenance_bind_for_package(string $backupRoot, array $packageSummary): array
{
    ...
    $expectedRel = trim((string) ($packageSummary['relative_package_path'] ?? ''));
    $sidecar = orange_backup_provenance_read_package(
        $backupRoot,
        $type,
        $packageId,
    ...
    $sidecarRel = (string) ($sidecar['relative_package_path'] ?? '');
    if ($expectedRel !== '' && $sidecarRel !== '' && $expectedRel !== $sidecarRel) {
        $packageSummary['provenance'] = null;
        $packageSummary['provenance_unavailable'] = true;
        $packageSummary['provenance_unavailable_reason'] = 'relative_path_mismatch';
        $packageSummary['provenance_message_ar'] = ORANGE_BACKUP_PROVENANCE_UNAVAILABLE_AR;
    ...
    $packageSummary['provenance'] = [
        ...
        'relative_package_path' => $sidecarRel,
...
function orange_backup_provenance_package_identity_fingerprint(
    ...
    if ($packageType === 'country' || $packageType === 'country_recovery') {
        $manifest = orange_backup_provenance_read_json_file($packagePath . DIRECTORY_SEPARATOR . 'manifest.json');
...
/**
 * Permission-safe Backend view model for later UI (Stage 1: not rendered).
 *
 * @param array<string, mixed> $admin
 * @return array<string, mixed>
 */
function orange_backup_provenance_resolve_for_viewer(
    string $backupRoot,
    array $admin,
    PDO $pdo,
    string $packageType,
    ...
    require_once __DIR__ . DIRECTORY_SEPARATOR . 'backup_admin.php';
    ...
    $denied = static function (string $code): array {
        return [
            ...
            'provenance_message_ar' => ORANGE_BACKUP_PROVENANCE_UNAVAILABLE_AR,
            'grants_execution' => false,
            'grants_verify' => false,
            'grants_drv' => false,
            'grants_restore' => false,
            'grants_cutover' => false,
            'grants_delete' => false,
        ];
    };
    ...
    if ($normalizedType === 'full') {
        $normalizedType = 'full_disaster';
    }
    if ($normalizedType === 'country') {
        $normalizedType = 'country_recovery';
    }

    $isSuper = orange_admin_is_superuser($admin);

    if ($normalizedType === 'full_disaster') {
        // Full provenance: existing Full/global authorized viewer only.
        if (!$isSuper) {
            return $denied('full_global_required');
        }
        // Superuser may_view is array-gated (no permission-matrix DB).
        if (!orange_backup_admin_may_view($admin, $pdo)) {
            return $denied('forbidden');
        }
    } elseif ($normalizedType === 'country_recovery') {
        // Country package filter first (matches existing Admin Country package visibility).
        try {
            orange_backup_admin_assert_country_package_in_context($pdo, (string) $countryCode);
        } catch (Throwable) {
            return $denied('country_scope_denied');
        }
        if ($isSuper) {
            if (!orange_backup_admin_may_view($admin, $pdo)) {
                return $denied('forbidden');
        ...
        // Country-scoped non-superuser: context match is sufficient for provenance read
        // (does not call permission-matrix / INFORMATION_SCHEMA; grants no execution rights).
    } else {
        return $denied('unsupported_package_type');
    }

    $bound = orange_backup_provenance_bind_for_package($backupRoot, [
        'package_type' => $normalizedType,
        'package_id' => $packageId,
        'country_code' => $countryCode ?? '',
        'package_fingerprint' => $packageFingerprint,
        'relative_package_path' => $relativePackagePath,
    ]);

    return [
        'ok' => empty($bound['provenance_unavailable']),
        'code' => empty($bound['provenance_unavailable']) ? 'ok' : (string) ($bound['provenance_unavailable_reason'] ?? 'unavailable'),
        'provenance' => $bound['provenance'] ?? null,
        'provenance_unavailable' => !empty($bound['provenance_unavailable']),
        'provenance_unavailable_reason' => $bound['provenance_unavailable_reason'] ?? null,
        'provenance_message_ar' => (string) ($bound['provenance_message_ar'] ?? ORANGE_BACKUP_PROVENANCE_UNAVAILABLE_AR),
        'grants_execution' => false,
        'grants_verify' => false,
        'grants_drv' => false,
        'grants_restore' => false,
...
/**
 * Parent Batch view for global authorized viewers (Stage 1 helper; not UI).
 *
 * @param array<string, mixed> $admin
 * @return array<string, mixed>
 */
function orange_backup_provenance_resolve_execution_for_viewer(
    string $backupRoot,
    array $admin,
    PDO $pdo,
    string $executionId
    ...
    // Global/superuser gate first — avoids Country-admin permission-matrix DB work for parent Batch.
    ...
    return [
        ...
        'execution' => [
            ...
            'trigger_mode' => (string) ($exec['trigger_mode'] ?? ''),
            'backup_scope' => (string) ($exec['backup_scope'] ?? ''),
            'initiated_by_kind' => (string) ($exec['initiated_by_kind'] ?? ''),
            'initiated_by_display_name_snapshot' => (string) ($exec['initiated_by_display_name_snapshot'] ?? ''),
            'initiated_context_country_code' => $exec['initiated_context_country_code'] ?? null,
            'overall_status' => (string) ($exec['overall_status'] ?? ''),
            'children' => is_array($exec['children'] ?? null) ? $exec['children'] : [],
            'started_at_utc' => (string) ($exec['started_at_utc'] ?? ''),
            'completed_at_utc' => (string) ($exec['completed_at_utc'] ?? ''),
        ],
        'grants_execution' => false,
    ];
}
