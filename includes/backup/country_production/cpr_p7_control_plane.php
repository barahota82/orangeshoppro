...
function orange_cpr_p7_control_plane_snapshot(): array
{
    return [
        ...
        'p7_integration_baseline_complete' => true,
...
function orange_cpr_p7_control_plane_assert(array $env): array
{
    ...
    if (empty($snap['drill_harness_implemented'])
        ...
        return [
            ...
            'message' => 'WP-P7-02…WP-P7-05 engines/freeze must be marked implemented (harness, execution, evidence, integration baseline).',
            'fail_closed' => true,
        ];
    }
    if (!empty($snap['owner_cert_pass_granted']) || !empty($snap['p8_started'])) {
        return [
            'ok' => false,
            'code' => 'p7_engines_premature',
            'message' => 'WP-P7-05 must not grant Owner Cert PASS or start P8.',
            'fail_closed' => true,
        ];
    }
    ...
    return [
        'ok' => true,
        'code' => 'ok',
        'message' => 'P7 control plane hard rules hold; clone-drill evidence baseline frozen; P8/enablement/Enterprise Audit/Tag withheld.',
        'snapshot' => $snap,
        'enablement_flag_observed' => false,
        'production_mutation' => false,
        'production_sql_executed' => false,
