...
function orange_catalog_ensure_schema_fast_path_slice(PDO $pdo): void
{
    ...
    /*
     * معاينة المنتج قبل النشر (docs/archive/ORANGE_PRODUCT_PREPUBLISH_PREVIEW_ROLLOUT.txt):
     * يجب أن تُطبَّق هذه الأعمدة على **مسار الويب السريع** (لا _core فقط) كي تُرحَّل تلقائياً بعد git pull
     * دون CLI — لأن HTTP لا يستدعي orange_catalog_ensure_schema_core. idempotent + إبطال كاش الأعمدة.
     */
    if (orange_table_exists($pdo, 'products') && !orange_table_has_column($pdo, 'products', 'is_preview_draft')) {
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD COLUMN is_preview_draft TINYINT(1) NOT NULL DEFAULT 0');
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD INDEX idx_products_is_preview_draft (is_preview_draft)');
        orange_schema_invalidate_column_check('products', 'is_preview_draft');
    }
    if (orange_table_exists($pdo, 'products') && !orange_table_has_column($pdo, 'products', 'preview_admin_id')) {
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD COLUMN preview_admin_id INT NULL DEFAULT NULL');
        orange_schema_invalidate_column_check('products', 'preview_admin_id');
    }
    if (orange_table_exists($pdo, 'products') && !orange_table_has_column($pdo, 'products', 'preview_token')) {
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD COLUMN preview_token VARCHAR(64) NULL DEFAULT NULL');
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD INDEX idx_products_preview_token (preview_token)');
        orange_schema_invalidate_column_check('products', 'preview_token');
    }
    if (orange_table_exists($pdo, 'products') && !orange_table_has_column($pdo, 'products', 'preview_source_product_id')) {
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD COLUMN preview_source_product_id INT NULL DEFAULT NULL');
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD INDEX idx_products_preview_owner (preview_admin_id, preview_source_product_id)');
        orange_schema_invalidate_column_check('products', 'preview_source_product_id');
    }
    if (orange_table_exists($pdo, 'products') && !orange_table_has_column($pdo, 'products', 'preview_expires_at')) {
        orange_catalog_safe_exec($pdo, 'ALTER TABLE products ADD COLUMN preview_expires_at DATETIME NULL DEFAULT NULL');
        orange_schema_invalidate_column_check('products', 'preview_expires_at');
    }
    orange_catalog_ensure_suppliers_schema($pdo);
    if (orange_table_exists($pdo, 'company_settings') && !orange_table_has_column($pdo, 'company_settings', 'vat_rate')) {
...
/**
 * توحيد قيمة/تكلفة التوصيل على مستوى المحافظة + علامات تبعية لكل مبلغ على المنطقة (v101).
 *
 * - delivery_governorates: default_delivery_fee / default_company_delivery_cost (NULL = غير محدّد).
 * - delivery_areas: fee_follows_gov / cost_follows_gov (0 = مخصّصة، 1 = تتبع المحافظة).
 *   الافتراضي 0 لحماية الصفوف الحالية (تُعامَل كمخصّصة لا تتأثر بتعديل المحافظة).
 */
function orange_catalog_migrate_delivery_gov_default_amounts_v101(PDO $pdo): void
{
    require_once __DIR__ . '/schema_migrations.php';

    $marker = 'php_delivery_gov_default_amounts_v101';
    if (orange_schema_migration_already_applied($pdo, $marker)) {
        return;
    }

    if (orange_table_exists($pdo, 'delivery_governorates')) {
        if (!orange_table_has_column($pdo, 'delivery_governorates', 'default_delivery_fee')) {
            orange_catalog_safe_exec(
                $pdo,
                'ALTER TABLE delivery_governorates ADD COLUMN default_delivery_fee DECIMAL(18,4) NULL DEFAULT NULL'
            );
            orange_schema_invalidate_column_check('delivery_governorates', 'default_delivery_fee');
        }
        if (!orange_table_has_column($pdo, 'delivery_governorates', 'default_company_delivery_cost')) {
            orange_catalog_safe_exec(
                $pdo,
                'ALTER TABLE delivery_governorates ADD COLUMN default_company_delivery_cost DECIMAL(18,4) NULL DEFAULT NULL'
            );
            orange_schema_invalidate_column_check('delivery_governorates', 'default_company_delivery_cost');
        }
    }

    if (orange_table_exists($pdo, 'delivery_areas')) {
        if (!orange_table_has_column($pdo, 'delivery_areas', 'fee_follows_gov')) {
            orange_catalog_safe_exec(
                $pdo,
                'ALTER TABLE delivery_areas ADD COLUMN fee_follows_gov TINYINT(1) NOT NULL DEFAULT 0'
            );
            orange_schema_invalidate_column_check('delivery_areas', 'fee_follows_gov');
        }
        if (!orange_table_has_column($pdo, 'delivery_areas', 'cost_follows_gov')) {
            orange_catalog_safe_exec(
                $pdo,
                'ALTER TABLE delivery_areas ADD COLUMN cost_follows_gov TINYINT(1) NOT NULL DEFAULT 0'
            );
...
/**
 * v106 — التحكّم بظهور السعر القديم المشطوب للعميل (قرار مالك 2026-06-28):
 *  - علم `show_old_price_to_customer` (افتراضي 0 = مُغلق لأسباب الترخيص) لجداول العروض المرتكزة على منتج:
 *    offers (عرض المنتج)، cart_combo_promotions (إجمالي المكوّنات مقابل سعر الحزمة)،
 *    cart_bogo_promotions (سعر تجزئة الهدية مقابل المجاني/المخفّض).
 *  - عند 0: لا يظهر السعر القديم. عند 1: يظهر القديم مشطوباً بجوار الجديد.
 * marker-gated + idempotent؛ يُستدعى من المسار الكامل والمسار السريع (نفس درس v105).
 */
function orange_catalog_migrate_promo_show_old_price_v106(PDO $pdo): void
{
    require_once __DIR__ . '/schema_migrations.php';

    $marker = 'php_promo_show_old_price_to_customer_v106';
    if (orange_schema_migration_already_applied($pdo, $marker)) {
        return;
    }
    ...
    foreach ($tables as $table) {
        ...
        if (!orange_table_has_column($pdo, $table, 'show_old_price_to_customer')) {
            orange_catalog_safe_exec(
                $pdo,
                'ALTER TABLE ' . $table . '
                    ADD COLUMN show_old_price_to_customer TINYINT(1) NOT NULL DEFAULT 0'
            );
            orange_schema_invalidate_column_check($table, 'show_old_price_to_customer');
        }
    }
...
/**
 * v107 — منتج مستهدف لعرض BOGO «نفس المنتج» (قرار مالك 2026-06-28):
 *  - عمود `same_variant_product_id` على `cart_bogo_promotions` (افتراضي NULL).
 *  - عند ضبطه: قاعدة same_variant تُطابق فقط إذا بلغت كمية ذلك المنتج تحديداً
 *    الحدّ الأدنى (min_buy_qty)، وتظهر بطاقة/صفحة عرض لذلك المنتج.
 *  - عند NULL/0: السلوك القديم (أي منتج يبلغ الحدّ الأدنى) — توافق رجعي كامل.
 * marker-gated + idempotent؛ يُستدعى من المسار الكامل والمسار السريع (نفس درس v106).
 */
function orange_catalog_migrate_promo_bogo_same_variant_product_v107(PDO $pdo): void
{
    require_once __DIR__ . '/schema_migrations.php';

    $marker = 'php_promo_bogo_same_variant_product_v107';
    if (orange_schema_migration_already_applied($pdo, $marker)) {
        return;
    }

    if (orange_table_exists($pdo, 'cart_bogo_promotions')
        && !orange_table_has_column($pdo, 'cart_bogo_promotions', 'same_variant_product_id')) {
        orange_catalog_safe_exec(
            $pdo,
