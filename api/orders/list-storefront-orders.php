...
try {
    ...
    $aid = (int) $acc['id'];
    $accCountryId = (int) ($acc['country_id'] ?? 0);
    $lang = current_lang();
    if ($bucket === 'delivered') {
    ...
    $params = [$aid];
    if ($accCountryId > 0 && orange_table_has_column($pdo, 'orders', 'country_id')) {
    ...
    foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
        if ($accCountryId > 0 && !isset($r['country_id'])) {
            $r['country_id'] = $accCountryId;
        }
