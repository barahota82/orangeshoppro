...
if ($hasInvCol) {
    $sql = "
        SELECT o.id, o.order_number, o.invoice_number, o.customer_name, o.phone, o.total, o.created_at
        FROM orders o
        WHERE o.invoice_number IS NOT NULL
          AND o.invoice_number <> ''
          AND o.invoice_number LIKE 'INV-C-%'
          AND o.order_source = 'company'
    ";
    $params = [];
...
<div class="admin-fy-shell" dir="rtl">
    <h1 class="admin-fy-shell__title">فواتير مبيعات (INV-C)</h1>
    <p class="admin-fy-shell__lead">
        فواتير الشركة من «فاتورة مبيعات» — بحث وتصفّح.
        <a href="<?php echo htmlspecialchars($indexBase . '?page=manual_order', ENT_QUOTES, 'UTF-8'); ?>">+ فاتورة مبيعات</a>
    </p>
...
<div class="card admin-fy-card">
    <form method="get" action="<?php echo htmlspecialchars($indexBase, ENT_QUOTES, 'UTF-8'); ?>" class="admin-toolbar" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;margin-bottom:12px;">
        <input type="hidden" name="page" value="sales_invoices">
    ...
    <?php elseif ($rows === []): ?>
    <p class="muted">لا توجد فواتير مبيعات (INV-C) بعد.</p>
    <?php else: ?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr<?php echo $oid === $navId ? ' style="background:#f0fdf4;"' : ''; ?>>
                    ...
                    <td><a href="<?php echo htmlspecialchars($indexBase . '?page=sales_invoices&id=' . $oid . ($q !== '' ? '&q=' . rawurlencode($q) : ''), ENT_QUOTES, 'UTF-8'); ?>">← →</a></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <div class="card-hint" style="margin-top:16px;padding:12px;border:1px solid #e2e8f0;border-radius:8px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:8px;">
            <?php if ($prevId > 0): ?>
            <a class="btn-secondary" href="<?php echo htmlspecialchars($indexBase . '?page=sales_invoices&id=' . $prevId . ($q !== '' ? '&q=' . rawurlencode($q) : ''), ENT_QUOTES, 'UTF-8'); ?>">← السابق</a>
            <?php endif; ?>
            <?php if ($nextId > 0): ?>
            <a class="btn-secondary" href="<?php echo htmlspecialchars($indexBase . '?page=sales_invoices&id=' . $nextId . ($q !== '' ? '&q=' . rawurlencode($q) : ''), ENT_QUOTES, 'UTF-8'); ?>">التالي →</a>
            <?php endif; ?>
