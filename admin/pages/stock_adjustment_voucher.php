...
<div class="admin-fy-shell" dir="rtl" id="stk_adj_app">
    <div class="page-title stk-print-hide">
    ...
    <p class="card-hint stk-print-hide" style="margin:0 0 12px;">
    ...
    <div class="card">
        ...
        <div class="form-grid">
            <div class="stk-header-line" style="grid-column:1/-1;display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));align-items:end;">
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="stk_notes" placeholder="ملاحظة السند (اختياري)">
                </div>
                <div class="stk-nav-cell stk-print-hide">
                    <div class="stk-nav-btns" role="group" aria-label="تنقل بين السندات">
                        <button type="button" class="btn-secondary stk-nav-btn" id="stk_nav_first" title="أول سند">&lt;&lt;</button>
                        <button type="button" class="btn-secondary stk-nav-btn" id="stk_nav_prev" title="السابق">&lt;</button>
                        <button type="button" class="btn-secondary stk-nav-btn" id="stk_nav_next" title="التالي">&gt;</button>
                        <button type="button" class="btn-secondary stk-nav-btn" id="stk_nav_last" title="آخر سند">&gt;&gt;</button>
                        <button type="button" class="btn-secondary" id="stk_btn_search" title="بحث عن سند">بحث</button>
                    </div>
                </div>
            </div>
        </div>

        <p id="stk_status_badge" class="card-hint" style="margin:6px 0 0;"></p>

        <div class="admin-doc-frame" style="margin-top:12px;">
            <div class="table-wrap">
                <table class="admin-table jv-lines-table stk-lines-table" id="stk_lines_table">
                    <thead>
                        <tr>
                            <th style="width:9rem;">كود الصنف</th>
                            <th>اسم الصنف</th>
                            <th style="width:8rem;">لون/مقاس</th>
                            <th style="width:7rem;">الرصيد الحالي</th>
                            <th style="width:6.5rem;">إضافة (+)</th>
                            <th style="width:6.5rem;">خصم (−)</th>
                            <th style="width:7rem;">تكلفة الوحدة</th>
                            <th style="width:7rem;">قيمة الفرق</th>
                            <th style="width:13rem;">حساب المعالجة</th>
                            <th>ملاحظة</th>
        ...
        <p id="stk_lines_empty" class="card-hint">لا أصناف — اضغط «+ سطر» ثم انقر نقرتين على خانة الكود لاختيار الصنف.</p>
        ...
        <div class="actions stk-print-hide" style="margin-top:16px;">
            <button type="button" class="btn-secondary" id="stk_btn_add">+ سطر</button>
            <button type="button" class="btn-secondary" id="stk_btn_new">سند جديد</button>
            <button type="button" class="btn-secondary btn-danger" id="stk_delete_btn">حذف المسودة</button>
            <button type="button" class="btn-secondary" id="stk_btn_print" disabled title="اعتمد السند أولاً">طباعة</button>
    ...
    <div id="stk_search_modal" class="stk-search-modal stk-print-hide" style="display:none;" aria-hidden="true" role="dialog">
    ...
    <div class="gl-pick-modal stk-print-hide" id="stk_acc_modal" hidden aria-hidden="true">
...
.stk-nav-btns { display: flex; gap: 6px; flex-wrap: wrap; }
.stk-nav-btn { min-width: 2.4rem; }
...
@media print {
    .stk-print-hide { display: none !important; }
}
</style>
...
(function () {
    ...
    function recompute() {
        ...
        el('stk_lines_disp').textContent = String(state.lines.length);
        el('stk_value_disp').textContent = fmt(total);
    }
    ...
    function render() {
        ...
        el('stk_lines_empty').style.display = state.lines.length ? 'none' : 'block';
