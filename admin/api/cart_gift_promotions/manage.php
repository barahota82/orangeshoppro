...
try {
    ...
    if ($action === 'save') {
        ...
        if ($id > 0) {
            ...
            $st = $pdo->prepare(
                'INSERT INTO cart_gift_promotions (country_id, name_ar, name_en, show_name_to_customer, min_subtotal, requires_registered_account, first_delivered_order_only, gift_kind, fixed_variant_id, pool_variant_ids, gift_unit_charge_kind, gift_unit_charge_value, sort_order, is_active, is_always_on, valid_from, valid_to) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            $st->execute([
                $insertCountryId,
                $nameAr,
