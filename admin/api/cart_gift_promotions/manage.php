...
try {
    ...
    if ($action === 'save') {
        ...
        if ($id > 0) {
            $st = $pdo->prepare(
                'UPDATE cart_gift_promotions SET min_subtotal = ?, requires_registered_account = ?, gift_kind = ?, fixed_variant_id = ?, pool_variant_ids = ?, sort_order = ?, is_active = ? WHERE id = ?'
            );
            $st->execute([
                $minSub,
                $reqReg,
            ...
            $st = $pdo->prepare(
                'INSERT INTO cart_gift_promotions (min_subtotal, requires_registered_account, gift_kind, fixed_variant_id, pool_variant_ids, sort_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)'
            );
            $st->execute([
                $minSub,
                $reqReg,
