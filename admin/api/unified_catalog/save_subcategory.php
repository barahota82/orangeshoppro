...
try {
    ...
    if ($slugRaw === '') {
        json_response(['success' => false, 'message' => 'slug غير صالح.'], 422);
    }
    if ($nameAr === '' || $nameEn === '') {
    ...
    $dupSlug->execute([$categoryId, $slugRaw, max(0, $id)]);
    if ($dupSlug->fetchColumn()) {
        json_response(['success' => false, 'message' => 'slug مستخدم تحت نفس الفئة الموحّدة.'], 409);
    }
