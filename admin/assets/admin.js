...
/**
 * صلاحيات مجموعة (warehouse، accounting، …).
 * @param {string} [resource]
 * @returns {{can_view:boolean,can_edit:boolean,can_delete:boolean,can_lock:boolean,can_unlock:boolean}}
 */
function orangeAdminCaps(resource) {
    var res = resource || window.ORANGE_ADMIN_PAGE_RESOURCE || 'dashboard';
...
/**
 * إخفاء/تعطيل عنصر إذا لم تتوفر الصلاحية.
 * @param {Element|null} el
 * @param {'view'|'edit'|'delete'|'lock'|'unlock'} perm
 * @param {string} [resource]
 */
function orangeAdminApplyPerm(el, perm, resource) {
    if (!el) {
        return;
    }
    var caps = orangeAdminCaps(resource);
    var key = 'can_' + perm;
    if (caps[key]) {
        return;
    }
...
/**
 * تطبيق data-orange-perm على العناصر داخل جذر (أو document).
...
function orangeAdminApplyPermMarkers(root) {
    ...
    scope.querySelectorAll('[data-orange-perm]').forEach(function (el) {
        orangeAdminApplyPerm(el, el.getAttribute('data-orange-perm') || 'view', el.getAttribute('data-orange-resource') || undefined);
    });
}
