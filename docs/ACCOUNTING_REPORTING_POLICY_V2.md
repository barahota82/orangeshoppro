...
- **Empty COA (no posting leaves yet):** Income statement, trading account, monthly and year-compare P&amp;L screens, **trial balance**, **financial report (consolidated)** and **single-account monthly GL**, **`partner_account_statement`**, **`partner_reports`**, **`opening_balances`**, **journal voucher screens** (manual / receipt / payment / other — `admin/pages/journal_voucher_screen.php`), **`journal_voucher_reports`**, **`accounting_reports_index`**, **`gl_posting`**, **`gl_account_settings`**, and **partner receipt/payment** (`admin/pages/partner_party_voucher_ui.php`) must still render the chrome (title, filters where applicable) and a visible in-app notice — not a blank main area — while the ledger has no posting-leaf rows to classify.
