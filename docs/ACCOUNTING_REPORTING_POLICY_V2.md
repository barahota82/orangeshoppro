...
- **Empty COA (no posting leaves yet):** Income statement, trading account, monthly and year-compare P&amp;L screens, **trial balance**, **financial report (consolidated)** and **single-account monthly GL**, and **`partner_account_statement` (standalone account statement)** must still render the chrome (title, filters) and a visible in-app notice — not a blank main area — while the ledger has no posting-leaf rows to classify.
