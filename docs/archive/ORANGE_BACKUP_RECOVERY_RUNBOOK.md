...
**Owner CRITICAL UX — guided operator workflow (Owner 2026-07-24 — presentation only):** Restore Center is **not** a dashboard of sections. It is a **sequential guided operator workflow**. At every moment exactly **one** step is the current actionable step; completed = ✔, current = ▶, future = 🔒, blocked = reason shown. Primary CTA is determined by UI from existing public job/package flags only (`resolveGuidedWorkflow` / `renderGuidedWorkflow` in `admin/pages/restore_center.php`). Stage action dumps (`jobActions` in active job / job history) are demoted; legacy stage/validation panels remain under «مراجع إضافية» only. **تنفيذ:** UI only — **no** Engine / Framework / APIs / Workers / Orchestrator / status transitions / Safety Gates / approval / password / maintenance / production logic changes. Workflow order includes Shadow Files (required by existing gates) between verify and smoke.
