...
**Owner UX — wizard cancel action (Owner 2026-07-24 — presentation only):** Every cancelable Restore Wizard step shows secondary **إلغاء المهمة** on the same horizontal action row opposite the primary CTA (outlined/neutral; never stronger than primary). Visibility follows framework `job.cancellable` only (`orange_restore_fw_cancellable_statuses`: queued / preparing / waiting_confirmation). After cancellation is no longer allowed (including once production mutation has begun), the button is absent. Click uses existing `rc-fw-cancel` → `admin/api/restore/job/cancel.php` with the existing confirm dialog. **تنفيذ:** `admin/pages/restore_center.php` UI only — no Engine/Framework/API/Workers/gates/transition changes.
