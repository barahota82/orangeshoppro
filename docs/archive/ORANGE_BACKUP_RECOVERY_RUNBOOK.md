...
**Stage 4A — Qualification evidence Backend (Owner 2026-08-05 — Backend only; no Backup Center visual change):** Durable Full Verify sibling `{package_id}.full_verify_report.json` beside `snapshots/` (not package payload). Package+action locks under `{BackupRoot}/.orange_meta/qualification_locks/v1/`. Bound read of Country Verify / Full DRV / Country DRV evidence with fingerprint/checksum digest validation; historical reports fail-closed when binding is insufficient. Server-side Verify/DRV idempotency short-circuits valid bound success (no heavy rerun; no duplicate audit). Normalized read-only resolver `orange_backup_qualification_resolve()`; Recoverable for future Stage 4B equals `orange_restore_admin_package_eligibility()` (rules unchanged). Stage 1 provenance remains outside qualification authority. **تنفيذ:** `includes/backup/backup_qualification.php` + `admin/api/backup/verify.php` / `recovery-check.php` lock/idempotency wiring + binding fields on Verify/DRV writers. **اختبار:** `scripts/self_test_backup_center_stage4a_qualification_backend.php`. Stage 4B (UI states/no-refresh) not started.
...
**Owner Architecture — Restore Center self-contained orchestration (Owner 2026-07-23 / corrected 2026-07-24; production hardening 2026-07-24; stage-safe v4 2026-07-24):** Restore Center is the **only** operational interface for restore. Operators must **not** use SSH/terminal/manual CLI. Existing approved workers remain the executors for post–Step-6 stages; Restore Center **schedules** them via `admin/api/restore/job/run-worker.php` + `includes/backup/restore/restore_center_orchestrator.php` (allowlisted `scripts/backup/*` with `--job=` only). **Detached spawn (mandatory for orchestrated workers):** HTTP must **not** wait for the worker; full stdin/stdout/stderr redirect (Windows `<NUL` + `>>log 2>&1` via launch cmd; Linux `</dev/null` + `>>log 2>&1` + `nohup`); operator may close the browser safely; progress by polling job status. Synchronous HTTP-supervised wait is **rejected for orchestrated workers**. **Server-side stage validation (mandatory):** inside the same mutex critical section as the execution claim, the orchestrator rejects schedule unless the current job status explicitly allows that worker (`restore_center_invalid_stage`); UI visibility / client state / allowlist alone are insufficient. **Atomic duplicate protection (orchestration layer):** one job + one stage = one running worker via per-stage mutex (`flock LOCK_EX|LOCK_NB`) + run claim (`orchestrator_{worker}.run.json`); claim lifecycle reconciles with job status (not PID-only) and must not remain indefinitely after stage completion. **No false success:** audit `restore_center_worker_scheduled` and API `scheduled:true` only after spawn + PID liveness verification; otherwise `restore_center_worker_schedule_failed` / `scheduled:false`. **Diagnostics:** `admin/api/restore/job/orchestrator-diagnostics.php` + Restore Center «تشخيص التنسيق» (safe operational reasons; redacted log tails; no secrets/absolute paths). Production Cutover Authorization via existing APIs. **No** restore engine / gate / transition / worker script rewrites.

**Owner Architecture — Restore Step 6 single Full Backup engine (Owner 2026-08-10; zero-legacy closure 2026-08-10):** Step 6 (mandatory pre-restore Full backup) must **not** use a parallel orchestrator launcher / `run-worker` / `_launch.cmd` / Step-6-only PHP discovery path. **One authoritative Full Backup service:** `orange_backup_execute_full_authoritative()` (CLI capture of `scripts/backup/run_full_backup.php` → `orange_backup_run_full()`). **Callers:** Backup Center `admin/api/backup/run-full.php` → `orange_backup_admin_run_full_for_api()`; Restore Step 6 `admin/api/restore/job/request-pre-restore-backup.php` → `orange_restore_admin_fw_execute_pre_restore_backup()` → same shared service, then verify/DRV/retention-pin and bind exact package identity to the Restore job before `pre_restore_backup_ready`. Execution mode: **BACKUP_CENTER_SYNCHRONOUS_SHARED_SERVICE** (HTTP remains open until shared service returns; same CLI-capture timeout as Backup Center). No internal HTTP to `run-full.php`. Schema remains **124**. Historical failed-job evidence is retained. Obsolete Step-6 launcher catalog entry is removed. Obsolete CLI worker `scripts/backup/restore_prepare_backup.php` is **deleted** (zero production callers; Admin uses the shared service adapter only). Retention pin reason `pre_restore_rollback_anchor` is created on ready and is **intentionally not auto-unpinned** on cancel/finalize (forensic; see finalize `retention_pin_removed=false`). **تنفيذ:** `includes/backup/backup_admin.php`, `includes/backup/restore/restore_pre_restore_backup.php`, `includes/backup/restore_admin.php`, `admin/api/restore/job/request-pre-restore-backup.php`, `includes/backup/restore/restore_center_orchestrator.php` (catalog cleanup), `includes/backup/restore/restore_production_cli_policy.php` (allowlist cleanup), `admin/pages/restore_center.php` (Step-6 UI calls shared endpoint).

**Owner Architecture — Step 6 failed retry + all-stage action execution lock (Owner 2026-08-11):** Explicit Step-6 retry from `pre_restore_backup_failed` must use **failed → pending → running** only (never `failed → running`). Audit order: request/pending commit → running commit → `pre_restore_backup_started` → shared engine. Latest-attempt diagnostics prefer current-path Step-6 events over historical `restore_center_worker_schedule_failed`. Owner UI must not expose raw `illegal_framework_status_transition…` tokens. Across all 16 guided Restore stages, every mutation action uses one shared client execution lock (immediate `disabled` + grey busy) with server-authoritative reconciliation after Refresh; network ambiguity never blind-reenables. Registers: `RESTORE_CENTER_STEP6_FAILED_RETRY_TRANSITION_01`, `RESTORE_CENTER_STEP6_FALSE_STARTED_AUDIT_EVENT_01`, `RESTORE_CENTER_STEP6_STALE_LEGACY_DIAGNOSTIC_01`, `RESTORE_CENTER_ALL_STAGE_ACTION_EXECUTION_LOCK_01`, `RESTORE_CENTER_ACTION_DOUBLE_SUBMIT_PREVENTION_01`, `RESTORE_CENTER_ACTION_LOCK_SERVER_RECONCILIATION_01`. **تنفيذ:** `includes/backup/restore/restore_pre_restore_backup.php`, `includes/backup/restore/restore_center_orchestrator.php`, `includes/backup/restore_admin.php`, `admin/api/restore/job/request-pre-restore-backup.php`, `admin/pages/restore_center.php`; tests `scripts/self_test_restore_center_step6_failed_retry_transition.php`, `scripts/self_test_restore_center_all_stage_action_lock.php`, `scripts/self_test_restore_center_action_lock_genuine_route.php`.

**Owner P0 — Shared Backup lock lifecycle across Backup Center + Restore Step 6 (Owner 2026-08-11):** Backup Center Full and Countries remain authoritative; Step 6 stays a thin adapter to `orange_backup_execute_full_authoritative()`. Lock release must succeed on every success/failure/timeout path (runner `finally` + `register_shutdown_function`, Admin ownership-aware reclaim after CLI timeout/failure, Step-6 `finally` for `.pre_restore_backup.lock`). PID liveness is `alive|dead|unknown` — **never** treat unknown/unprobeable as alive. Orphan Full/Country locks with dead PID are reclaimable immediately; unknown liveness may reclaim after **900s** (`ORANGE_BACKUP_LOCK_UNVERIFIED_STALE_SECONDS`); absolute stale remains **21600s**. Active proven owners are never deleted. Restore-only options (`forbid_latest_snapshot_refresh`) must not become Backup Center defaults. Backup Center UI frozen. Registers: `BACKUP_CENTER_AUTHORITATIVE_BEHAVIOR_FREEZE_01`, `BACKUP_CENTER_FULL_REGRESSION_P0_01`, `BACKUP_CENTER_COUNTRIES_REGRESSION_P0_01`, `BACKUP_RESTORE_SHARED_LOCK_LIFECYCLE_P0_01`, `RESTORE_STEP6_THIN_ADAPTER_ONLY_01`. **تنفيذ:** `includes/backup/backup_environment.php`, `includes/backup/backup_runner.php`, `includes/backup/country_batch_export.php`, `includes/backup/backup_admin.php`, `includes/backup/restore/restore_pre_restore_backup.php`; test `scripts/self_test_backup_restore_cross_surface_shared_lock_p0.php`. Evidence (outside Git): `D:\orange_backup_restore_cross_surface_p0_evidence\`.
...
**Owner P0 — True Backup Center execution baseline recovery (Owner 2026-08-11):** True Owner-accepted Backup Center baseline is Commit `b47cbe86` (not `d570e563`). Restore-era absolute PHP CLI policy must not govern Backup Center Full/Countries. **Contracts:** Backup Center → `orange_backup_admin_resolve_cli_php_binary()` (b47cbe86: ORANGE_PHP_CLI | PHP_BINARY/+cgi-sibling | bare `php` fallback; never throws). Restore detached workers → `orange_restore_worker_resolve_cli_php_binary()` in `includes/backup/restore/restore_worker_php_cli.php` (absolute-only). Read-only diagnostic remains POST `runtime-diagnostic.php` and lists proven Arabic blockers (`owner_blocker_list_ar` / «العوائق المثبتة»). Step 6 stays Phase-1 frozen. Registers: `BACKUP_CENTER_RUNTIME_ENGINE_NOT_READY_01`, `BACKUP_CENTER_DIAGNOSTIC_BLOCKER_DETAIL_INCOMPLETE_01`, `BACKUP_CENTER_WRONG_RECOVERY_BASELINE_01`, `BACKUP_CENTER_TRUE_KNOWN_GOOD_BASELINE_REQUIRED_01`. **تنفيذ:** `includes/backup/backup_admin.php`, `includes/backup/restore/restore_worker_php_cli.php`, `includes/backup/restore/restore_center_orchestrator.php`, `includes/backup/backup_runtime_diagnostic.php`; tests `scripts/self_test_backup_center_true_baseline_recovery.php`, updated Step6 resolver disposition tests. Evidence (outside Git): `D:\orange_backup_true_baseline_recovery_evidence\`.

**Owner P0 — Restore Center Step 6 Restore-only adapter reactivation (Phase 2, Owner 2026-08-11):** After live Full + Countries success and diagnostic `READY_FOR_CONTROLLED_FULL_ATTEMPT`, remove the temporary Step 6 Phase-1 freeze via a Restore-only thin adapter that calls the immutable Backup Center Full callable `orange_backup_admin_run_full_for_api()` (same path as `admin/api/backup/run-full.php` → CLI `scripts/backup/run_full_backup.php`). Restore owns auth/CSRF/perms/job/state/per-job lock/invoke/exact package bind/pin/audit/dialog; Restore does **not** own Backup dump/CLI/global Full lock/package/manifest engines. Backup Center manual default behavior remains unchanged; Step 6 calls the shared callable with `refresh_snapshot_after_cli=false` so a successful Backup result must include an explicit package id. Exact package ID from Backup result required — no Restore-side latest-directory guess and no Backup Center latest-snapshot fallback for Step 6. Same live job retry remains Owner-only after Result A auto-deploy. Registers: `RESTORE_CENTER_STEP6_RESTORE_ONLY_ADAPTER_01`, `BACKUP_CENTER_IMMUTABLE_PRODUCTION_BOUNDARY_01`, `RESTORE_STEP6_EXACT_BACKUP_RESULT_BINDING_01`, `RESTORE_STEP6_NO_BACKUP_BACKEND_MODIFICATION_01`, `RESTORE_STEP6_SAME_LIVE_JOB_RETRY_REQUIRED_01`. **تنفيذ:** `admin/api/restore/job/request-pre-restore-backup.php`, `includes/backup/restore/restore_pre_restore_backup.php`, `includes/backup/restore_admin.php`, `includes/backup/backup_admin.php` (opt-out fallback flag, default unchanged); permanent tests under `scripts/self_test_restore_center_step6_*` + Backup blob freeze suite. Evidence (outside Git): `D:\orange_restore_step6_restore_only_phase2_evidence\`.
