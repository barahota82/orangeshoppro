...
**Owner UX — Restore Step 1 package view modes latest-5 / all (Owner 2026-07-24 — presentation + list scope):** Step 1 package card supports two UI modes (Backup Center pattern): default **آخر 5 حزم مؤهلة** and **عرض جميع الحزم** / **العودة إلى آخر 5 حزم** at the bottom of the same card (no modal/page). Country packages follow Admin Country Context only; Full packages remain global. Latest-5 is applied **after** eligibility + country scope + newest-first ordering (never a global limit before filter). Mode is client UI state only; cancel / country-context reload resets to latest-5. Eligibility policy unchanged. **تنفيذ:** `admin/pages/restore_center.php` + minimal `admin/api/restore/list.php` country-context scoping (same as Backup Center `list_country_packages(..., null, $countryContextCode)`).
