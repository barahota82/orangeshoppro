...
**Owner P0 — Restore Center Step 6 Restore-only adapter reactivation (Phase 2, Owner 2026-08-11):** After live Full + Countries success and diagnostic `READY_FOR_CONTROLLED_FULL_ATTEMPT`, remove the temporary Step 6 Phase-1 freeze via a Restore-only thin adapter that calls the immutable Backup Center Full callable `orange_backup_admin_run_full_for_api()` unchanged (same path as `admin/api/backup/run-full.php` → CLI `scripts/backup/run_full_backup.php`). Restore owns auth/CSRF/perms/job/state/per-job lock/invoke/exact package bind/pin/audit/dialog; Restore does **not** modify Backup Center Production files or own Backup dump/CLI/global Full lock/package/manifest engines. Exact package ID from Backup result required — no Restore-side latest-directory guess. Same live job retry remains Owner-only after Result A auto-deploy. Registers: `RESTORE_CENTER_STEP6_RESTORE_ONLY_ADAPTER_01`, `BACKUP_CENTER_IMMUTABLE_PRODUCTION_BOUNDARY_01`, `RESTORE_STEP6_EXACT_BACKUP_RESULT_BINDING_01`, `RESTORE_STEP6_NO_BACKUP_BACKEND_MODIFICATION_01`, `RESTORE_STEP6_SAME_LIVE_JOB_RETRY_REQUIRED_01`. **تنفيذ:** `admin/api/restore/job/request-pre-restore-backup.php`, `includes/backup/restore/restore_pre_restore_backup.php`, `includes/backup/restore_admin.php`; permanent tests under `scripts/self_test_restore_center_step6_*` + Backup blob freeze suite. Evidence (outside Git): `D:\orange_restore_step6_restore_only_phase2_evidence\`.
