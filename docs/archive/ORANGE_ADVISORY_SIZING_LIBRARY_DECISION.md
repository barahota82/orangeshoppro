...
**خلاصة النقاش:** فلترة العائلات **بالقسم لا معنى لها** لأن العائلة الواحدة (مثلاً «ملابس علوي ألفا S/M/L») **مشتركة** بين رجالي/حريمي/أطفال؛ والفرق بينها يكون في **الدليل** (القياسات) لا في العائلة. كما أن الاشتقاق من القسم يعتمد على وسم أنواع المنتجات وهو غير مكتمل أثناء التجهيز. القرار النهائي: **تُفلتَر العائلات بنوعها هي** (`commercial_kind_key` + `sizing_category_key` المسجّلان على العائلة وقت إنشائها) عبر قائمتي تصفية في نموذج الدليل:
- **«نوع المقاس»** (`asg_kind_filter`): قيمها من الأنواع التجارية المتوفّرة بين العائلات، مُسمّاة من `commercial_kind_dictionary`.
- **«الفئة»** (`asg_cat_filter`): تظهر بعد اختيار نوع، قيمها من فئات عائلات ذلك النوع، مُسمّاة من `sizing_category_dictionary`.

اختيار «— كل الأنواع —» يعرض الكل. يفصل هذا الملابس عن الأحذية/الإكسسوار (وهو المطلوب الفعلي) بلا اعتماد على بيانات القسم. **القسم يبقى في النموذج للربط والتخزين فقط، لا للفلترة.** **واجهة فقط** — بلا تغيير مخطط/API/متجر.
...
- **فلترة العائلة حسب نوعها (2026-06-23 — يلغي محاولة فلترة القسم في 2026-06-22):** في نفس الشاشة فقط — استعلام العائلات يقرأ `commercial_kind_key` + `sizing_category_key`؛ قواميس PHP `kindLabels` (من `commercial_kind_dictionary`) و`catLabels` (من `sizing_category_dictionary`) تُمرَّر JSON (`KIND_LABELS` + `CAT_LABELS`)؛ JS: `rebuildKindFilter()`/`rebuildCatFilter()` تبنيان قائمتي التصفية من أنواع/فئات العائلات نفسها، `familyAllowed(fam)` تطابق نوع/فئة العائلة المختارة، `rebuildFamilyOptions()` تعرض المطابق (أو الكل عند «كل الأنواع»). القسم لم يعد يفلتر العائلات؛ `asg_dept.onchange` يحدّث أهداف الربط فقط. `loadGuide` يصفّر الفلاتر كي تظهر العائلة المحفوظة. أُزيلت خرائط `DEPT_KINDS/DEPT_FAMILIES` وخانة `asg_family_all`. بلا تغيير مخطط/API/متجر.
