...
4. **`config/country_restore_boundary_matrix.json`** — Phase C3 runtime matrix for CRP **export** (derived from items 1–3)  
5. **`COUNTRY_CRP_EXPORT_ENGINE_C3.md`** — C3 export engine notes (export only; no restore)  
6. **`COUNTRY_RESTORE_ARCHITECTURE.md`** — historical C0 narrative only  
7. `backup_table_registry.json` — operational registry; must be corrected to match schema+this policy before execution; until then schema+this policy win on conflicts  
