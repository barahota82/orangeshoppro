...
/**
 * Phase 3B.3B4 — CLI worker: restore Full package SQL into shadow DB only.
 *
 * Usage:
 *   php scripts/backup/restore_shadow_db.php --job=JOB_ID
 *
...
if ($jobId === '' || !preg_match('/^[a-zA-Z0-9._-]+$/', $jobId)) {
    fwrite(STDERR, "Usage: php restore_shadow_db.php --job=JOB_ID\n");
    exit(2);
}
...
try {
    ...
    $result = orange_restore_shadow_run_cli($projectRoot, $workRoot, $backupRoot, $jobId, 'cli');

    if (!empty($result['ok'])) {
        echo 'SHADOW_RESTORE_RESULT: PASS' . PHP_EOL;
