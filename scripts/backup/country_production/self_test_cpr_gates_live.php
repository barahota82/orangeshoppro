...
function cpr_gl_setup_ready(array $env): array
{
    ...
    // Scaffold runbook + CP5 (G27/G28) — witnesses live in WP-P4-08; predicates require checkpoints.
...
try {
    cpr_gl('scaffold_version', ORANGE_CPR_SCAFFOLD_VERSION === 'P4-09-integration-baseline');
    ...
    // C8 change also breaks fingerprint — validate_evidence checks C8 SAFE before hash loop for c8...
    // Actually order: C8 SAFE check first, then hash loop. UNSAFE → gatelive_c8_not_safe.
    ...
    // --- Suite FAIL path (evidence enablement false → G01 FAIL, still seals live report) ---
