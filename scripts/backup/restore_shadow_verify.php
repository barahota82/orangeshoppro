...
/**
 * Phase 3B.3B5 — CLI worker: deep-verify restored shadow DB readiness.
 *
 * Usage:
 *   php scripts/backup/restore_shadow_verify.php --job=JOB_ID
 *
...
if ($jobId === '' || !preg_match('/^[a-zA-Z0-9._-]+$/', $jobId)) {
    fwrite(STDERR, "Usage: php restore_shadow_verify.php --job=JOB_ID\n");
    exit(2);
}
...
try {
    ...
    $result = orange_restore_shadow_verify_run_cli($projectRoot, $workRoot, $backupRoot, $jobId, 'cli');
    $overall = strtoupper((string) ($result['result'] ?? 'FAIL'));

    echo 'SHADOW_VERIFY_RESULT: ' . $overall . PHP_EOL;
    ...
    if ($overall === 'READY' || $overall === 'WARNING' || !empty($result['ok'])) {
        exit(0);
    }

    echo 'CODE: ' . (string) ($result['code'] ?? 'shadow_not_ready') . PHP_EOL;
    exit(1);
} catch (Throwable $e) {
    $code = trim($e->getMessage()) ?: 'shadow_verification_failed';
    fwrite(STDERR, 'ERROR: ' . $code . PHP_EOL);
    echo 'SHADOW_VERIFY_RESULT: FAIL' . PHP_EOL;
