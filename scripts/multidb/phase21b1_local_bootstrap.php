...
/**
 * Orange Phase 2.1B.1 Central Identity — local disposable Control Trust-Core bootstrap.
 *
 * Loopback MySQL only. Creates Control DB + four principals (× localhost + 127.0.0.1):
 *   p21b1_sa_* (schema_admin/DEFINER), p21b1_tr_* (trust_executor),
 *   p21b1_id_* (identity_executor), p21b1_rt_* (control_runtime).
 * Secrets only in %TEMP%. Never prints passwords. No Country DB. No Production touch.
 *
 * Env:
 *   ORANGE_PHASE21B1_MYSQL_ADMIN_USER (default: root)
 *   ORANGE_PHASE21B1_MYSQL_ADMIN_PASS (default: empty)
 *   ORANGE_PHASE21B1_MYSQL_HOST (default: 127.0.0.1)
 *   ORANGE_PHASE21B1_MYSQL_PORT (default: 3306)
 *
 * Usage:
 *   php scripts/multidb/phase21b1_local_bootstrap.php
 *   php scripts/multidb/phase21b1_local_bootstrap.php --run-id=abc123
 */
...
$host = getenv('ORANGE_PHASE21B1_MYSQL_HOST');
$host = ($host !== false && trim((string) $host) !== '') ? trim((string) $host) : '127.0.0.1';
$port = getenv('ORANGE_PHASE21B1_MYSQL_PORT');
$port = ($port !== false && trim((string) $port) !== '') ? (int) $port : 3306;
$adminUser = getenv('ORANGE_PHASE21B1_MYSQL_ADMIN_USER');
$adminUser = ($adminUser !== false && trim((string) $adminUser) !== '') ? trim((string) $adminUser) : 'root';
$adminPassEnv = getenv('ORANGE_PHASE21B1_MYSQL_ADMIN_PASS');
...
function p21b1_assert_local_host(string $host): void
{
...
function p21b1_sql_ident(string $name): string
{
...
function p21b1_sql_string(string $value): string
{
    return "'" . str_replace(["\\", "'"], ["\\\\", "\\'"], $value) . "'";
}

function p21b1_random_password(int $bytes = 24): string
{
 ...
 */
function p21b1_attempt_secret_acl(string $path): string
{
...
p21b1_assert_local_host($host);

$controlDb = 'orange_phase21b1_control_' . strtolower($runId);
$saUser = 'p21b1_sa_' . strtolower($runId);
$trUser = 'p21b1_tr_' . strtolower($runId);
$idUser = 'p21b1_id_' . strtolower($runId);
$rtUser = 'p21b1_rt_' . strtolower($runId);
$saPass = p21b1_random_password();
$trPass = p21b1_random_password();
$idPass = p21b1_random_password();
$rtPass = p21b1_random_password();
...
$statePath = $tempDir . DIRECTORY_SEPARATOR . 'orange_phase21b1_state_' . $runId . '.json';
$secretsPath = $tempDir . DIRECTORY_SEPARATOR . 'orange_phase21b1_secrets_' . $runId . '.json';
...
try {
    ...
    $admin->exec('CREATE DATABASE IF NOT EXISTS orange_phase21b1_capprobe CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
    $admin->exec('USE orange_phase21b1_capprobe');
    ...
    $probeUser = 'p21b1_cap_' . strtolower($runId);
    $probePass = p21b1_random_password();
    $columnGrantOk = 0;
    $procedureOk = 0;
    $triggerSignalOk = 0;
    $grantExecuteOk = 0;
    try {
        foreach ($hostIdents as $hi) {
            $admin->exec('CREATE USER ' . p21b1_sql_string($probeUser) . '@' . p21b1_sql_string($hi)
                . ' IDENTIFIED BY ' . p21b1_sql_string($probePass));
            $admin->exec('GRANT SELECT (id), UPDATE (a) ON orange_phase21b1_capprobe.t_col_probe TO '
                . p21b1_sql_string($probeUser) . '@' . p21b1_sql_string($hi));
        }
    ...
    try {
        ...
        $definerRow = $admin->query(
            "SELECT DEFINER FROM information_schema.ROUTINES
             WHERE ROUTINE_SCHEMA='orange_phase21b1_capprobe' AND ROUTINE_NAME='p_cap_definer'"
        ...
        foreach ($hostIdents as $hi) {
            $admin->exec(
                'GRANT EXECUTE ON PROCEDURE orange_phase21b1_capprobe.p_cap_definer TO '
                . p21b1_sql_string($probeUser) . '@' . p21b1_sql_string($hi)
            );
        }
        $admin->exec('FLUSH PRIVILEGES');
    ...
    foreach ($hostIdents as $hi) {
        try {
            $admin->exec('DROP USER IF EXISTS ' . p21b1_sql_string($probeUser) . '@' . p21b1_sql_string($hi));
        } catch (Throwable $e) {
            // ignore
        }
    }
    $admin->exec('DROP DATABASE IF EXISTS orange_phase21b1_capprobe');
    ...
    if ($generatedOk !== 1 || $procedureOk !== 1 || $triggerSignalOk !== 1 || $grantExecuteOk !== 1) {
        fwrite(STDERR, "PHASE21B1_LOCAL_MYSQL_CAPABILITY_BLOCKER\n");
        exit(4);
    }
    if ($columnGrantOk !== 1) {
        fwrite(STDERR, "PHASE21B1_LOCAL_MYSQL_CAPABILITY_BLOCKER column_level_grant_unsupported\n");
    ...
    $localOverbroadSa = 0;
    foreach ($hostIdents as $hi) {
        $admin->exec('CREATE USER ' . p21b1_sql_string($saUser) . '@' . p21b1_sql_string($hi)
            . ' IDENTIFIED BY ' . p21b1_sql_string($saPass));
    }
    try {
        foreach ($hostIdents as $hi) {
            $u = p21b1_sql_string($saUser) . '@' . p21b1_sql_string($hi);
        ...
        $localOverbroadSa = 1;
        foreach ($hostIdents as $hi) {
            $admin->exec('GRANT ALL PRIVILEGES ON *.* TO ' . p21b1_sql_string($saUser) . '@' . p21b1_sql_string($hi)
                . ' WITH GRANT OPTION');
        }
        $admin->exec('FLUSH PRIVILEGES');
    }
    ...
    $admin->exec('CREATE DATABASE ' . p21b1_sql_ident($controlDb) . ' CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
    foreach ($hostIdents as $hi) {
        $u = p21b1_sql_string($saUser) . '@' . p21b1_sql_string($hi);
        $db = p21b1_sql_ident($controlDb);
    ...
    $sa->exec('USE ' . p21b1_sql_ident($controlDb));
    if ($binlogTrust !== 1) {
        // Fall back: install as admin so DEFINER=admin, then still fail-closed if SA cannot own routines.
        fwrite(STDERR, "PHASE21B1_LOCAL_MYSQL_CAPABILITY_BLOCKER binlog_trust\n");
        exit(4);
    }
    orange_control_ensure_schema($sa, [
        'ownership_token' => $ownershipToken,
        'ownership_run_id' => strtolower($runId),
        'installed_by' => 'phase21b1_bootstrap',
        'allow_meta_insert' => true,
    ]);

    $physHash = orange_control_physical_manifest_hash($sa, $controlDb, 3);
    ...
    $superHash = password_hash('Phase21B1!SuperFixture', PASSWORD_DEFAULT);
    $sa->exec(
    ...
    foreach (
        ...
        foreach ($hostIdents as $hi) {
            $sa->exec('CREATE USER ' . p21b1_sql_string($u) . '@' . p21b1_sql_string($hi)
                . ' IDENTIFIED BY ' . p21b1_sql_string($p));
        }
    }

    $db = p21b1_sql_ident($controlDb);
    $adminNs = 'id,admin_uuid,username,display_name,is_active,is_superuser,'
        . 'is_global_access,disabled_at,password_changed_at,created_at,updated_at,row_version';
    $sessNs = 'id,admin_id,created_at,expires_at,last_seen_at,idle_timeout_sec,'
        . 'selected_country_id,selected_country_uuid,ip_hash,user_agent_hash,revoked_at,revoke_reason';
    ...
    foreach ($hostIdents as $hi) {
        $u = p21b1_sql_string($trUser) . '@' . p21b1_sql_string($hi);
    ...
    foreach ($hostIdents as $hi) {
        $u = p21b1_sql_string($idUser) . '@' . p21b1_sql_string($hi);
    ...
    foreach ($hostIdents as $hi) {
        $u = p21b1_sql_string($rtUser) . '@' . p21b1_sql_string($hi);
        foreach (
            [
                'ctrl_countries',
                'ctrl_db_host_profiles',
    ...
    $grantModel = 'definer;positive_col_allowlist;no_db_star_select_id_tr_rt;'
        . 'tr_trust_exec;id_identity_callable_no_emit;rt_select_audit;'
        . 'deny_password_hash;deny_session_token_hash;deny_token_fingerprint;'
        . 'deny_ownership_token;no_identity_dml;no_orange_db';
    ...
    $state = [
        ...
        'phase21b1_active_registry_count' => $activeRegistry,
    ...
    $aclStatus = p21b1_attempt_secret_acl($secretsPath);
    ...
    echo 'PHASE21B1_ACTIVE_REGISTRY_COUNT=' . $activeRegistry . "\n";
