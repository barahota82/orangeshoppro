...
/**
 * Orange Phase 2.1B.1 Central Identity — cleanup disposable Control DB and principals for one run.
 *
 ...
 *
 * Usage:
 *   php scripts/multidb/phase21b1_local_cleanup.php --run-id=<id>
 *   php scripts/multidb/phase21b1_local_cleanup.php --state=<path-to-state.json>
 *
 * Env (same as bootstrap): ORANGE_PHASE21B1_MYSQL_ADMIN_USER/PASS/HOST/PORT
 */
...
function p21b1_cleanup_assert_local_host(string $host): void
{
...
function p21b1_cleanup_sql_ident(string $name): string
{
...
function p21b1_cleanup_sql_string(string $value): string
{
    return "'" . str_replace(["\\", "'"], ["\\\\", "\\'"], $value) . "'";
}

p21b1_cleanup_assert_local_host($host);
...
if ($statePathArg !== null && $statePathArg !== '') {
    ...
    $candidate = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'orange_phase21b1_state_' . $runId . '.json';
    if (is_file($candidate)) {
        ...
        $state = [
            'run_id' => $runId,
            'control_db' => 'orange_phase21b1_control_' . strtolower($runId),
            'schema_admin_user' => 'p21b1_sa_' . strtolower($runId),
            'trust_executor_user' => 'p21b1_tr_' . strtolower($runId),
            'identity_executor_user' => 'p21b1_id_' . strtolower($runId),
            'control_runtime_user' => 'p21b1_rt_' . strtolower($runId),
...
if ($controlDb === '' || !preg_match('/^orange_phase21b1_control_[a-z0-9]+$/', $controlDb)) {
...
try {
    ...
    $dbExists = (int) $admin->query(
        ...
        . p21b1_cleanup_sql_string($controlDb)
    )->fetchColumn();
    ...
    if ($dbExists === 1) {
        ...
        $admin->exec('USE ' . p21b1_cleanup_sql_ident($controlDb));
        $metaTok = $admin->query(
    ...
    foreach ([$saUser, $trUser, $idUser, $rtUser] as $u) {
        if ($u === '' || !preg_match('/^p21b1_(sa|tr|id|rt)_[a-z0-9]+$/', $u)) {
            continue;
        }
        foreach ($hostIdents as $hi) {
            try {
                $admin->exec(
                    'DROP USER IF EXISTS ' . p21b1_cleanup_sql_string($u) . '@' . p21b1_cleanup_sql_string((string) $hi)
                );
            } catch (Throwable $e) {
                // continue
            }
    ...
    $admin->exec('DROP DATABASE IF EXISTS ' . p21b1_cleanup_sql_ident($controlDb));
    $admin->exec('FLUSH PRIVILEGES');

    $dbRem = (int) $admin->query(
        'SELECT COUNT(*) FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = ' . p21b1_cleanup_sql_string($controlDb)
    )->fetchColumn();
    ...
    $nearPrefix = (int) $admin->query(
        "SELECT COUNT(*) FROM information_schema.SCHEMATA
         WHERE SCHEMA_NAME LIKE 'orange_phase21b1_controlxx_%'
            OR SCHEMA_NAME LIKE 'orange_phase21b1_control\\_%' ESCAPE '\\\\'
            AND SCHEMA_NAME <> " . p21b1_cleanup_sql_string($controlDb)
    )->fetchColumn();
    ...
    $stateFile = isset($state['state_path']) ? (string) $state['state_path'] : (
        $runIdFinal !== ''
            ? sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'orange_phase21b1_state_' . $runIdFinal . '.json'
            : null
    );
